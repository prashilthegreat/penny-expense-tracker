...
export default function Home() {
  ...
  const [editingId, setEditingId] = useState<number | null>(null); const [editingTitle, setEditingTitle] = useState("");
  ...
  function beginTitleEdit(expense: Expense) { setEditingId(expense.id); setEditingTitle(expense.merchant); }
  function saveTitleEdit(id: number) {
    const merchant = editingTitle.trim(); if (!merchant) { setEditingId(null); setEditingTitle(""); return; }
    setExpenses(old => { const next = old.map(item => item.id === id ? { ...item, merchant } : item); localStorage.setItem("penny-expenses", JSON.stringify(next)); return next; });
    setEditingId(null); setEditingTitle(""); setToast("Transaction title updated"); setTimeout(() => setToast(""), 2500);
  }
